...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "WITH RECURSIVE birthday_years AS (SELECT 1990 AS year UNION ALL SELECT year + 1 FROM birthday_years WHERE year < 2024) SELECT year AS год, CASE CAST(strftime('%w', year || '-03-15') AS INTEGER) WHEN 0 THEN 'Воскресенье' WHEN 1 THEN 'Понедельник' WHEN 2 THEN 'Вторник' WHEN 3 THEN 'Среда' WHEN 4 THEN 'Четверг' WHEN 5 THEN 'Пятница' WHEN 6 THEN 'Суббота' END AS день_недели FROM birthday_years"
echo " "
